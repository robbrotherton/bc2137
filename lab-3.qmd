...
Notice that valid responses for the "undermine the news media" concern item are 1 (not at all concerned) through 5 (extremely concerned). I want higher scores on that question to indicate greater concern, so that's fine. 
...
my_data_complete <- my_data_complete |>
  ...
  select(-trust) # don't need the original trust column any more

```


Now I have my two variables, concern about the news media, and (reverse-coded) distrust of the news media, for each of the `r scales::label_comma()(nrow(my_data_complete))` participants with complete data. We're ready to start exploring the data.
...
There are many ways of doing this, but for now I'll just use the `mean()` and `sd()` functions. I can refer to a particular column in a data.frame using the `$` operator, i.e. `my_data$concern` and so on.

```{r}
mean(my_data_complete$concern)
sd(my_data_complete$concern)
...
```{r}
#| label: fig-concern-no-theme
#| fig-cap: Histogram of responses to "concern about government undermining news media" question
...
#| fig-cap: Histogram of responses to concern about news being undermined
