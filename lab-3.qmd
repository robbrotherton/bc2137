...
To implement this as a `filter()`, we can use the column name, followed by the condition. So, `trust >= 0` is telling R "check each value in the `trust` column and tell me if it is greater than or equal to 0." Rows where this evaluates to `TRUE` will be kept, and rows for which it is `FALSE` will be dropped. We can do this for any columns we like, separating the arguments with commas. Rows for which all conditions are met will be retained in the resulting dataframe. [^2] 
